...
export const ConfigChange = {
  ...
  fromJSON(object: any): ConfigChange {
    return {
      ...
      changeType: isSet(object.changeType)
        ...
        : 0,
      advices: Array.isArray(object?.advices)
        ? object.advices.map((e: any) => Advice.fromJSON(e))
        : [],
    };
  },
  ...
  toJSON(message: ConfigChange): unknown {
    ...
    if (message.advices) {
      obj.advices = message.advices.map((e) =>
        e ? Advice.toJSON(e) : undefined
      );
    } else {
      obj.advices = [];
    }
  ...
  fromPartial<I extends Exact<DeepPartial<ConfigChange>, I>>(
    object: I
  ): ConfigChange {
    const message = createBaseConfigChange();
