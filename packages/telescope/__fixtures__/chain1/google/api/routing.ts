...
export const RoutingRule = {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): RoutingRule {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 2:
          message.routingParameters.push(
            RoutingParameter.decode(reader, reader.uint32())
          );
          break;
        default:
  ...
  toJSON(message: RoutingRule): unknown {
    ...
    if (message.routingParameters) {
      obj.routingParameters = message.routingParameters.map((e) =>
        e ? RoutingParameter.toJSON(e) : undefined
      );
    } else {
  ...
  fromPartial<I extends Exact<DeepPartial<RoutingRule>, I>>(
    object: I
  ): RoutingRule {
    const message = createBaseRoutingRule();
    message.routingParameters =
      object.routingParameters?.map((e) => RoutingParameter.fromPartial(e)) ||
      [];
    return message;
  },
};
...
export const RoutingParameter = {
  ...
  toJSON(message: RoutingParameter): unknown {
    ...
    message.pathTemplate !== undefined &&
      (obj.pathTemplate = message.pathTemplate);
    return obj;
  },

  fromPartial<I extends Exact<DeepPartial<RoutingParameter>, I>>(
    object: I
  ): RoutingParameter {
    const message = createBaseRoutingParameter();
