    ...
    toAmino: ({
      ...
      return {
        ...
        channel: {
          ...
          ordering: {
            order_id: {
              owner: channel.ordering.orderId.owner,
              dseq: channel.ordering.orderId.dseq.toString(),
              gseq: channel.ordering.orderId.gseq,
              oseq: channel.ordering.orderId.oseq
            },
            state: channel.ordering.state,
            spec: {
              name: channel.ordering.spec.name,
              requirements: {
                signed_by: {
                  all_of: channel.ordering.spec.requirements.signedBy.allOf,
                  any_of: channel.ordering.spec.requirements.signedBy.anyOf
                },
                attributes: channel.ordering.spec.requirements.attributes.map(el0 => ({
                  key: el0.key,
                  value: el0.value
                }))
              },
              resources: channel.ordering.spec.resources.map(el0 => ({
                resources: {
                  cpu: {
                    units: {
                      val: el0.resources.cpu.units.val
                    },
                    attributes: el0.resources.cpu.attributes.map(el1 => ({
                      key: el1.key,
                      value: el1.value
                    }))
                  },
                  memory: {
                    quantity: {
                      val: el0.resources.memory.quantity.val
                    },
                    attributes: el0.resources.memory.attributes.map(el1 => ({
                      key: el1.key,
                      value: el1.value
                    }))
                  },
                  storage: {
                    quantity: {
                      val: el0.resources.storage.quantity.val
                    },
                    attributes: el0.resources.storage.attributes.map(el1 => ({
                      key: el1.key,
                      value: el1.value
                    }))
                  },
                  endpoints: el0.resources.endpoints.map(el1 => ({
                    kind: el1.kind
                  }))
                },
                count: el0.count,
                price: {
                  denom: el0.price.denom,
                  amount: Long.fromNumber(el0.price.amount).toString()
                }
              }))
            },
            created_at: channel.ordering.createdAt.toString()
          },
          counterparty: {
