...
describe('imports lookup', () => {
    it('Any', () => {
        ...
        const { obj: _obj, ...Any } = importLookup(store, ref, 'Any');
    ...
    describe('google/api/expr/v1alpha1/checked', () => {
        ...
        it('Empty', () => {
            const { obj, ...Empty } = importLookup(store, ref, 'Empty');
            expect(obj.name).toEqual('Empty')
            expect(obj.scope).toEqual(['google.protobuf'])
            expect(Empty).toMatchSnapshot();
        });
        it('NullValue', () => {
            const { obj, ...NullValue } = importLookup(store, ref, 'NullValue');
            expect(obj.name).toEqual('NullValue')
            expect(obj.scope).toEqual(['google.protobuf'])
            expect(NullValue).toMatchSnapshot();
        });
        it('google.protobuf.NullValue', () => {
            const { obj, importedName, ...NullValue } = protoImportLookup(store, ref, 'google.protobuf.NullValue');
            expect(obj.name).toEqual('NullValue')
            expect(obj.scope).toEqual(['google.protobuf'])
        ...
        it('google.protobuf.Empty', () => {
            const { obj, importedName, ...Empty } = protoImportLookup(store, ref, 'google.protobuf.Empty');
            expect(obj.name).toEqual('Empty')
    ...
    describe('osmosis/claim/v1beta1/params.proto', () => {
        ...
        it('google.protobuf.Duration', () => {
            const { obj, importedName, ...Duration } = protoImportLookup(store, ref, 'google.protobuf.Duration');
        ...
        it('Duration', () => {
            const { obj, ...Duration } = importLookup(store, ref, 'Duration');
            expect(obj.name).toEqual('Duration')
            expect(obj.scope).toEqual(['google.protobuf'])
            expect(Duration).toMatchSnapshot();
        });
    ...
    it('cosmos.base.query.v1beta1.PageResponse', () => {
        ...
        const { obj, importedName, ...PageResponse } = protoImportLookup(store, ref, 'cosmos.base.query.v1beta1.PageResponse');
    ...
    describe('cosmos/tx/v1beta1/service.proto', () => {
        ...
        it('.tendermint.types.BlockID', () => {
            ...
            const { obj, importedName, ...BlockID } = protoImportLookup(store, ref, '.tendermint.types.BlockID');
        ...
        it('.tendermint.types.Block', () => {
            ...
            // https://github.com/pyramation/protobuf-example-reference/blob/master/ts/cosmos/tx/v1beta1/service.ts#L7-L8
            // https://github.com/pyramation/protobuf-example-reference/blob/master/json/cosmos/tx/v1beta1/service.json#L238-L242
            const { obj, importedName, ...Block } = protoImportLookup(store, ref, '.tendermint.types.Block');
    ...
    it('google.rpc.Status', () => {
        ...
        const { obj, importedName, ...Status } = protoImportLookup(store, ref, 'google.rpc.Status');
    ...
    it('ibc.core.client.v1.Height', () => {
        ...
        const { obj, importedName, ...Height } = protoImportLookup(store, ref, 'ibc.core.client.v1.Height');
