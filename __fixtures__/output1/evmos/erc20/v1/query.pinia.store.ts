...
export const usePiniaStore = defineStore('evmos/erc20/v1/query.proto', {
  state: () => {
    return {
      tokenPairs: ({} as QueryTokenPairsResponse),
      tokenPair: ({} as QueryTokenPairResponse),
  ...
  actions: {
    async fetchTokenPairs(param : QueryTokenPairsRequest) {
      this.tokenPairs = await this.lcdClient.tokenPairs(param);
      return this.tokenPairs;
    },

    async fetchTokenPair(param : QueryTokenPairRequest) {
