...
export class Query {
  static EpochInfos(request: QueryEpochsInfoRequest, initRequest?: fm.InitReq): Promise<QueryEpochsInfoResponse> {
    return fm.fetchReq(`/evmos/epochs/v1/epochs?${fm.renderURLSearchParams(request, [])}`, { ...initRequest,
      method: "GET"
    });
  }

  static CurrentEpoch(request: QueryCurrentEpochRequest, initRequest?: fm.InitReq): Promise<QueryCurrentEpochResponse> {
    return fm.fetchReq(`/evmos/epochs/v1/current_epoch?${fm.renderURLSearchParams(request, [])}`, { ...initRequest,
      method: "GET"
    });
  }
